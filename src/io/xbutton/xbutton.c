...
static void handle_events(app_input_evt_t evt, uint8_t button_idx)
{
  switch (evt)
  {
    case APP_INPUT_CLICK:
...
nrfx_err_t xbutton_enable(uint8_t button_idx, bool high_accuracy)
{
  ...
  app_input_on(APP_INPUT_CLICK, button_idx, handle_events);
  app_input_on(APP_INPUT_DOUBLE_CLICK, button_idx, handle_events);
  app_input_on(APP_INPUT_LONG_PRESS_START, button_idx, handle_events);
  app_input_on(APP_INPUT_LONG_PRESS_STOP, button_idx, handle_events);
